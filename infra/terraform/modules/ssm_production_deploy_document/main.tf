# Custom production SSM Command document (Early Access Phase 1).
...
# Never AWS-RunShellScript.

locals {
  document_content = {
    schemaVersion = "2.2"
    description   = "DealBrain production digest deploy. Bounded parameters; host-side secrets only."
    parameters = {
      ReleaseId = {
        type           = "String"
    ...
    mainSteps = [
      {
        ...
        name   = "DealBrainProductionDeploy"
        inputs = {
          ...
          runCommand = [
            ...
            "exec /opt/dealbrain/bin/dealbrain-production-deploy.sh",
          ]
        }
      },
    ]
