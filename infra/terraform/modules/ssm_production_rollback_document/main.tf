# Custom production SSM Command document for rollback (Early Access Phase 1).
...
# no RDS mutation/delete.

locals {
  document_content = {
    schemaVersion = "2.2"
    description   = "DealBrain production digest rollback. Bounded parameters; host-side secrets only. Does not modify RDS."
    parameters = {
      ReleaseId = {
        type           = "String"
      ...
      BundleBucket = {
        type           = "String"
        description    = "Production release-artifacts S3 bucket name."
        allowedPattern = "^dealbrain-production-release-artifacts-[0-9]{12}$"
      }
      BundleKey = {
        type           = "String"
    ...
    mainSteps = [
      {
        ...
        name   = "DealBrainProductionRollback"
        inputs = {
          ...
          runCommand = [
            ...
            "export DEALBRAIN_ENVIRONMENT='production'",
            "export DEALBRAIN_OPERATION='rollback'",
            "exec /opt/dealbrain/bin/dealbrain-production-rollback.sh",
          ]
        }
      },
    ]
...
resource "aws_ssm_document" "production_rollback" {
