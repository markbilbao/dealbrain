#!/bin/bash
# DealBrain production EC2 user_data bootstrap (Early Access Phase 1).
...
#!/bin/bash
# Install Docker Compose plugin on Amazon Linux 2023 (production host).
#
# Contract:
...
# Fixed safe-extract helper (mirrors scripts/deploy/verify_production_bundle.py contract).
# Updated from release bundles after first successful extract.
cat >/opt/dealbrain/bin/verify_production_bundle.py << 'SAFEEXTRACT'
#!/usr/bin/env python3
...
# Baseline bootstrap gate only. Rollback-specific host tooling is delivered by
# Deploy Production schema-2 bundles (not required for initial EC2 cloud-init).
REQUIRED_MEMBERS = (
...
cat >/opt/dealbrain/bin/dealbrain-production-deploy.sh << 'ENTRYPOINT'
#!/bin/bash
set -euo pipefail
set +x

die() { echo "[dealbrain-production-entrypoint] ERROR: $*" >&2; exit 1; }
...
mkdir -p "$LOCK_DIR"
exec 9>"$LOCK_FILE"
flock -w 30 9 || die "could not acquire production deploy lock within 30s"
export DEALBRAIN_LOCK_HELD=1

ORCH="${RELEASE_DIR}/bin/dealbrain-production-deploy.sh"
...
[[ -x /opt/dealbrain/bin/verify_production_bundle.py || -f /opt/dealbrain/bin/verify_production_bundle.py ]] \
  || die "verify_production_bundle.py missing"
python3 /opt/dealbrain/bin/verify_production_bundle.py "$TMP_BUNDLE" \
...
[[ -x "${RELEASE_DIR}/bin/dealbrain-production-deploy.sh" ]] || die "orchestrator missing in bundle"

# Refresh fixed helpers from release for subsequent deploys.
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/dealbrain-production-deploy.sh" \
  /opt/dealbrain/bin/dealbrain-production-deploy.orchestrator.sh
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/verify_production_bundle.py" \
  /opt/dealbrain/bin/verify_production_bundle.py 2>/dev/null || true
...
bash "${RELEASE_DIR}/bin/dealbrain-production-deploy.sh"
exit $?
ENTRYPOINT
chmod 0755 /opt/dealbrain/bin/dealbrain-production-deploy.sh

touch /opt/dealbrain/bootstrap.ok
chmod 0644 /opt/dealbrain/bootstrap.ok
echo "=== dealbrain production bootstrap ok $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
