"""Guard: Sprint 12 must not modify protected intelligence / ops modules."""
...
# SHA-256 digests of protected modules as of Sprint 12 branch creation (main).
PROTECTED_DIGESTS = {
    ...
    # Review Intelligence (Sprint 11) — protected for Sprint 12.
...
def test_review_summary_module_is_independent() -> None:
    """Review Summary may read Review Intelligence but must not rewrite it."""
    summary_service = ROOT / "app/services/review_summary_service.py"
    assert summary_service.is_file()
    text = summary_service.read_text(encoding="utf-8")
    assert "ReviewSummaryService" in text
    assert "Watchlist" not in text
    assert "CollectionOperations" not in text
    assert "DealScore" not in text

    review_service = ROOT / "app/services/review_service.py"
    assert "ReviewSummaryService" not in review_service.read_text(encoding="utf-8")

    ops = ROOT / "app/services/collection_operations_service.py"
    assert "ReviewSummaryService" not in ops.read_text(encoding="utf-8")

    watchlist = ROOT / "app/services/watchlist_service.py"
    assert "ReviewSummaryService" not in watchlist.read_text(encoding="utf-8")
