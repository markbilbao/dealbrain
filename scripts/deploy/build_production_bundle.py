#!/usr/bin/env python3
"""Build a deterministic production release bundle (Sprint 25b.3).

Includes only production compose overlays, host deploy scripts, the validated
original release manifest, and bundle-meta.json. Never includes staging
...
INCLUDE_FILES: tuple[tuple[str, str], ...] = (
    ...
    ("infra/compose/docker-compose.production.yml", "compose/docker-compose.production.yml"),
    ("scripts/deploy/host/dealbrain-production-deploy.sh", "bin/dealbrain-production-deploy.sh"),
    ("scripts/deploy/host/dealbrain-production-rollback.sh", "bin/dealbrain-production-rollback.sh"),
    ...
    ("scripts/deploy/host/verify-production.sh", "bin/verify-production.sh"),
    ("scripts/deploy/host/write-production-evidence.py", "bin/write-production-evidence.py"),
    (
        "scripts/deploy/host/write-production-rollback-evidence.py",
        "bin/write-production-rollback-evidence.py",
    ),
    (
        "scripts/deploy/host/resolve-production-rollback-migration.py",
        "bin/resolve-production-rollback-migration.py",
    ),
    ("scripts/deploy/production_evidence.py", "bin/production_evidence.py"),
    ("scripts/deploy/production_rollback_evidence.py", "bin/production_rollback_evidence.py"),
    ("scripts/deploy/prior_production_evidence.py", "bin/prior_production_evidence.py"),
    ("scripts/deploy/verify_host_production_rollback_tooling.py", "bin/verify_host_production_rollback_tooling.py"),
    ("scripts/deploy/log_redaction.py", "bin/log_redaction.py"),
    ("scripts/deploy/alb_target_health.py", "bin/alb_target_health.py"),
    ("scripts/deploy/verify_production_bundle.py", "bin/verify_production_bundle.py"),
    ("scripts/deploy/probe_checks.py", "bin/probe_checks.py"),
    ("schemas/production-deploy-evidence.schema.json", "bin/production-deploy-evidence.schema.json"),
    ("schemas/production-rollback-evidence.schema.json", "bin/production-rollback-evidence.schema.json"),
)
...
def build_bundle(
    *,
    ...
    try:
        ...
        # Fail closed if staging overlay somehow present.
        forbidden = staging / "compose" / "docker-compose.staging.yml"
        if forbidden.exists():
            raise BundleError("staging compose overlay must not be in production bundle")
