#!/bin/bash
# Production post-deploy verification gates (Sprint 25b.3 / 25b.4a / 25b.5h).
...
case "$TG_ARN" in
  *staging*) echo "ERROR: staging target group identifier" >&2; exit 1 ;;
esac
case "$TG_ARN" in
  *production*|*dealbrain-production*) ;;
  *) echo "ERROR: target group ARN must identify production" >&2; exit 1 ;;
esac
...
[[ "$APP_ENV_VAL" == "production" ]] || { echo "ERROR: APP_ENV is not production" >&2; exit 1; }
...
echo "ok: production verification gates passed"
