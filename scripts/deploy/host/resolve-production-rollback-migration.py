...
else use migration_revision_after from the exact fully validated production_ok
...
def _load_prior_module():
    sibling = Path(__file__).resolve().parent / "prior_production_evidence.py"
    if sibling.is_file():
        spec = importlib.util.spec_from_file_location(
            "dealbrain_prior_production_evidence_host", sibling
        )
        if spec is not None and spec.loader is not None:
            ...
            return module
    try:
        from scripts.deploy import prior_production_evidence as module

        return module
    except ImportError as exc:
        print("FAIL: prior_production_evidence module unavailable", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if migration is None or migration.authority != mod.MIGRATION_AUTHORITY_DEPLOY_VERSION:
        ...
        try:
            ...
            prior = mod.select_authoritative_prior_production_evidence(
                pairs,
