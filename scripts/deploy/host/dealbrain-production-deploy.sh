#!/bin/bash
# DealBrain production host deploy orchestrator (Early Access Phase 1).
# Invoked by SSM document DealBrain-ProductionDeploy via /opt/dealbrain/bin/.
# Secrets never printed; shell tracing disabled around secret material.
set -euo pipefail

umask 077

log() { echo "[dealbrain-production-deploy] $*"; }
die() { echo "[dealbrain-production-deploy] ERROR: $*" >&2; exit 1; }
...
[[ "$BUNDLE_BUCKET" =~ ^dealbrain-production-release-artifacts-[0-9]{12}$ ]] || die "invalid production BundleBucket"
...
LOCK_INFO="${LOCK_DIR}/production-deploy.lock.info"
EVIDENCE_DIR="${ROOT}/runtime/evidence"
COMPOSE_PROJECT=dealbrain-production
...
PROJECT_TAG="$(aws ec2 describe-tags --region "$REGION" \
  ...
  --query 'Tags[0].Value' --output text)"
[[ "$ENV_TAG" == "production" ]] || die "host Environment tag is not production (got ${ENV_TAG})"
[[ "$ROLE_TAG" == "api-compose-host" ]] || die "host Role tag mismatch"
[[ "$PROJECT_TAG" == "dealbrain" ]] || die "host Project tag mismatch"
case "$ENV_TAG" in
  *staging*) die "staging identifier on production host" ;;
esac
...
# Bound binder wait before production_ok evidence (poll interval × attempts).
...
resolve_ssm_command_id_for_evidence() {
  ...
  die "ssm_command_id unavailable for production_ok evidence (binder wait expired)"
}
...
# Reconciliation never claims production_ok.
RELEASE_COMMITTED=0
...
_evidence_py() {
  if [[ -f "${RELEASE_DIR}/bin/production_evidence.py" ]]; then
    echo "${RELEASE_DIR}/bin/production_evidence.py"
  elif [[ -f "${ROOT}/bin/production_evidence.py" ]]; then
    echo "${ROOT}/bin/production_evidence.py"
  else
    return 1
  fi
}
...
path = sys.argv[1]
spec = importlib.util.spec_from_file_location("dealbrain_production_evidence", path)
...
PY
  ...
  out="${EVIDENCE_DIR}/production-deploy-evidence-${RELEASE_ID}-${DEPLOY_RUN_ID}.json"

  local writer=""
  if [[ -f "${RELEASE_DIR}/bin/write-production-evidence.py" ]]; then
    writer="${RELEASE_DIR}/bin/write-production-evidence.py"
  elif [[ -f "${ROOT}/bin/write-production-evidence.py" ]]; then
    writer="${ROOT}/bin/write-production-evidence.py"
  else
  ...
  DEALBRAIN_ASSUMED_ROLE_ARN="arn:aws:iam::${AWS_ACCOUNT_ID}:role/dealbrain-production-gha-deploy" \
  DEALBRAIN_ROLE_SESSION_NAME="gha-${DEPLOY_RUN_ID}-production" \
  python3 "$writer" || return 1

  evidence_key="evidence/${RELEASE_ID}/${DEPLOY_RUN_ID}/production-deploy-evidence.json"
...
on_exit() {
  ...
  # Preserves original exit code; never emit production_ok + failure_reason.
  if [[ $code -ne 0 ]]; then
    FINAL_STATUS="failed"
...
if [[ "${DEALBRAIN_LOCK_HELD:-}" != "1" ]]; then
  ...
  if ! flock -w 30 9; then
    die "could not acquire production deploy lock within 30s"
  fi
fi
cat >"$LOCK_INFO" <<EOF
...
# -------------------------------------------------------------------------
SAFE_EXTRACT=""
if [[ -x "${ROOT}/bin/verify_production_bundle.py" ]]; then
  SAFE_EXTRACT="${ROOT}/bin/verify_production_bundle.py"
elif [[ -f "${ROOT}/bin/verify_production_bundle.py" ]]; then
  SAFE_EXTRACT="${ROOT}/bin/verify_production_bundle.py"
fi
...
if [[ "${DEALBRAIN_BUNDLE_ALREADY_EXTRACTED:-}" == "1" && -f "${RELEASE_DIR}/bundle-meta.json" ]]; then
  ...
  if [[ -n "$SAFE_EXTRACT" ]]; then
    ...
    [[ -f "${RELEASE_DIR}/compose/docker-compose.production.yml" ]] || die "production overlay missing"
    [[ ! -f "${RELEASE_DIR}/compose/docker-compose.staging.yml" ]] || die "staging overlay forbidden"
  fi
else
  ...
  if [[ -z "$SAFE_EXTRACT" ]]; then
    die "verify_production_bundle.py missing on host — refusing raw tar extract"
  fi
...
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/verify-production.sh" \
  "${ROOT}/bin/verify-production.sh"
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/write-production-evidence.py" \
  "${ROOT}/bin/write-production-evidence.py" 2>/dev/null || true
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/production_evidence.py" \
  "${ROOT}/bin/production_evidence.py" 2>/dev/null || true
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/log_redaction.py" \
  "${ROOT}/bin/log_redaction.py" 2>/dev/null || true
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/verify_production_bundle.py" \
  "${ROOT}/bin/verify_production_bundle.py" 2>/dev/null || true
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/deploy_atomicity.sh" \
  "${ROOT}/bin/deploy_atomicity.sh" 2>/dev/null || true
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/dealbrain-production-rollback.sh" \
  "${ROOT}/bin/dealbrain-production-rollback.sh" 2>/dev/null || true
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/write-production-rollback-evidence.py" \
  "${ROOT}/bin/write-production-rollback-evidence.py" 2>/dev/null || true
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/production_rollback_evidence.py" \
  "${ROOT}/bin/production_rollback_evidence.py" 2>/dev/null || true
install -o root -g root -m 0644 \
  "${RELEASE_DIR}/bin/production-deploy-evidence.schema.json" \
  "${ROOT}/bin/production-deploy-evidence.schema.json" 2>/dev/null || true
install -o root -g root -m 0644 \
  "${RELEASE_DIR}/bin/production-rollback-evidence.schema.json" \
  "${ROOT}/bin/production-rollback-evidence.schema.json" 2>/dev/null || true
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/prior_production_evidence.py" \
  "${ROOT}/bin/prior_production_evidence.py" 2>/dev/null || true
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/verify_host_production_rollback_tooling.py" \
  "${ROOT}/bin/verify_host_production_rollback_tooling.py" 2>/dev/null || true
install -o root -g root -m 0755 \
  "${RELEASE_DIR}/bin/resolve-production-rollback-migration.py" \
  "${ROOT}/bin/resolve-production-rollback-migration.py" 2>/dev/null || true

# Refresh trusted host rollback tooling capability inventory (preflight authority).
if [[ -f "${ROOT}/bin/verify_host_production_rollback_tooling.py" ]]; then
  python3 "${ROOT}/bin/verify_host_production_rollback_tooling.py" \
    --bin-dir "${ROOT}/bin" \
    --capability-path "${ROOT}/bin/production-host-tooling.json" \
    --expected-tooling-version "ea-p1" \
    --write \
    || die "failed to write production host tooling capability"
fi
...
COMPOSE_OVERLAY="${RELEASE_DIR}/compose/docker-compose.production.yml"
[[ -f "$COMPOSE_BASE" && -f "$COMPOSE_OVERLAY" ]] || die "compose overlays missing"
[[ ! -f "${RELEASE_DIR}/compose/docker-compose.staging.yml" ]] || die "staging compose overlay forbidden in production bundle"
...
python3 "${BIN}/assemble-runtime-env.py" \
  ...
  --secrets-prefix dealbrain/production

require_disk_gib "$PRE_PULL_MIN_GIB" "before-pull"
bash "${BIN}/ghcr-login.sh" --region "$REGION" --secret-id dealbrain/production/ghcr_pull --allow-production
...
[[ -n "$SSM_COMMAND_ID" ]] || {
  FAILURE_REASON="evidence_upload_ssm_command_id_missing"
  die "ssm_command_id unavailable for production_ok evidence"
}
...
log "production deploy succeeded for ${RELEASE_ID}"
