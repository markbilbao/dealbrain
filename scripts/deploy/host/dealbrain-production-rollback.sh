#!/bin/bash
# DealBrain production host rollback orchestrator (Early Access Phase 1).
# Invoked by SSM document DealBrain-StagingRollback via /opt/dealbrain/bin/.
# No image rebuild, no DB downgrade, no staging path. Secrets never printed.
set -euo pipefail

umask 077

log() { echo "[dealbrain-production-rollback] $*"; }
die() { echo "[dealbrain-production-rollback] ERROR: $*" >&2; exit 1; }
...
[[ "$BUNDLE_BUCKET" =~ ^dealbrain-production-release-artifacts-[0-9]{12}$ ]] || die "invalid BundleBucket"
...
TOOLING_PREFLIGHT="${ROOT}/bin/verify_host_production_rollback_tooling.py"
[[ -f "$TOOLING_PREFLIGHT" ]] || die "verify_host_production_rollback_tooling.py missing on host"
python3 "$TOOLING_PREFLIGHT" \
  --bin-dir "${ROOT}/bin" \
  --capability-path "${ROOT}/bin/production-host-tooling.json" \
  --expected-tooling-version "${DEALBRAIN_EXPECTED_TOOLING_VERSION:-ea-p1}" \
...
[[ "$BUNDLE_BUCKET" == "dealbrain-production-release-artifacts-${AWS_ACCOUNT_ID}" ]] \
...
_evidence_py() {
  if [[ -f "${TARGET_DIR}/bin/production_evidence.py" ]]; then
    echo "${TARGET_DIR}/bin/production_evidence.py"
...
normalize_alembic_revision() {
  local raw="$1"
  local evidence_py
  evidence_py="$(_evidence_py)" || die "production_evidence.py missing; cannot normalize alembic revision"
...
download_prior_evidence_candidates() {
  # Download every production-deploy-evidence.json AND its .sha256 sidecar.
  ...
  keys="$(aws s3api list-objects-v2 --bucket "$BUNDLE_BUCKET" --prefix "$prefix" \
    --query "Contents[?ends_with(Key, 'production-deploy-evidence.json')].Key" --output text 2>/dev/null || true)"
  [[ -n "$keys" && "$keys" != "None" ]] || return 1
  for key in $keys; do
    [[ "$key" == *"/production-deploy-evidence.json" ]] || continue
    base="$(basename "$(dirname "$key")")"
    run_dir="${dest}/${base}"
    mkdir -p "$run_dir"
    if ! aws s3 cp "s3://${BUNDLE_BUCKET}/${key}" \
      "${run_dir}/production-deploy-evidence.json" --region "$REGION" --only-show-errors >/dev/null 2>&1; then
      rm -rf "$run_dir"
      continue
    fi
    if ! aws s3 cp "s3://${BUNDLE_BUCKET}/${key}.sha256" \
      "${run_dir}/production-deploy-evidence.json.sha256" --region "$REGION" --only-show-errors >/dev/null 2>&1; then
      # Missing sidecar — leave incomplete pair; selector rejects it.
      rm -f "${run_dir}/production-deploy-evidence.json"
      rmdir "$run_dir" 2>/dev/null || true
      continue
    fi
  done
  # At least one JSON+sidecar pair must exist.
  compgen -G "${dest}/*/production-deploy-evidence.json" >/dev/null \
    && compgen -G "${dest}/*/production-deploy-evidence.json.sha256" >/dev/null
}

# --- BEGIN resolve_target_recorded_migration (test-extractable) ---
resolve_target_recorded_migration() {
  # Prefer DEPLOY_VERSION; fallback only to fully validated prior production_ok evidence.
  ...
  resolver=""
  if [[ -f "${ROOT}/bin/resolve-production-rollback-migration.py" ]]; then
    resolver="${ROOT}/bin/resolve-production-rollback-migration.py"
  elif [[ -f "${TARGET_DIR}/bin/resolve-production-rollback-migration.py" ]]; then
    resolver="${TARGET_DIR}/bin/resolve-production-rollback-migration.py"
  else
  ...
  case "$MIGRATION_AUTHORITY" in
    deploy_version|validated_prior_production_evidence) ;;
    *)
      TARGET_RECORDED_MIGRATION=""
      MIGRATION_AUTHORITY=""
      return 1
...
restore_source_api() {
  ...
  local overlay="${SOURCE_DIR}/compose/docker-compose.production.yml"
...
PY
  ...
  out="${EVIDENCE_DIR}/production-rollback-evidence-${TARGET_RELEASE_ID}-${ROLLBACK_RUN_ID}.json"

  local writer=""
  if [[ -f "${TARGET_DIR}/bin/write-production-rollback-evidence.py" ]]; then
    writer="${TARGET_DIR}/bin/write-production-rollback-evidence.py"
  elif [[ -f "${ROOT}/bin/write-production-rollback-evidence.py" ]]; then
    writer="${ROOT}/bin/write-production-rollback-evidence.py"
  else
  ...
  DEALBRAIN_ROLE_SESSION_NAME="gha-${ROLLBACK_RUN_ID}-production-rollback" \
  python3 "$writer" || return 1

  evidence_key="evidence/${TARGET_RELEASE_ID}/${ROLLBACK_RUN_ID}/production-rollback-evidence.json"
...
if [[ "${DEALBRAIN_LOCK_HELD:-}" != "1" ]]; then
  ...
  if ! flock -w 30 9; then
    die "could not acquire production deploy/rollback lock within 30s"
  fi
fi
cat >"$LOCK_INFO" <<EOF
...
# -------------------------------------------------------------------------
SAFE_EXTRACT=""
if [[ -f "${ROOT}/bin/verify_production_bundle.py" ]]; then
  SAFE_EXTRACT="${ROOT}/bin/verify_production_bundle.py"
fi

if [[ -f "${TARGET_DIR}/bundle-meta.json" && -f "${TARGET_DIR}/compose/docker-compose.production.yml" ]]; then
  ...
  [[ -n "$SAFE_EXTRACT" ]] || die "verify_production_bundle.py missing — refusing raw tar extract"
...
python3 "${ROOT}/bin/verify_production_bundle.py" \
...
COMPOSE_OVERLAY="${TARGET_DIR}/compose/docker-compose.production.yml"
[[ -f "$COMPOSE_BASE" && -f "$COMPOSE_OVERLAY" ]] || {
  FAILURE_REASON="target_release_incomplete"
  die "compose overlays missing"
}
[[ ! -f "${TARGET_DIR}/compose/docker-compose.staging.yml" ]] || die "staging compose forbidden"
...
for helper in \
  verify-production.sh ghcr-login.sh assemble-runtime-env.py production_evidence.py \
  verify_production_bundle.py log_redaction.py alb_target_health.py \
  production-deploy-evidence.schema.json
do
...
bash "$GHCR_LOGIN" --region "$REGION" --secret-id dealbrain/production/ghcr_pull --allow-production
...
VERIFY_STAGING="${ROOT}/bin/verify-production.sh"
[[ -f "$VERIFY_STAGING" ]] || VERIFY_STAGING="${TARGET_DIR}/bin/verify-production.sh"
[[ -f "$VERIFY_STAGING" ]] || die "verify-production.sh missing on host and target"
bash "$VERIFY_STAGING" \
  --env-file "$ENV_FILE" \
  --image-digest "$IMAGE_DIGEST" \
  --image-repository "$IMAGE_REPOSITORY" \
...
log "production rollback succeeded to ${TARGET_RELEASE_ID}"
rm -f "$LOCK_INFO"
trap on_exit EXIT
