#!/usr/bin/env python3
"""Validate production rollback eligibility before host mutation.

Authority is the immutable Build Image release-manifest plus production S3 state.
Mutable image tags are never accepted as authority.

Invoke from the repository root as a module:

  python -m scripts.deploy.validate_production_rollback_eligibility [options]
"""
...
from scripts.deploy.production_evidence import EvidenceError, normalize_alembic_revision
from scripts.deploy.prior_production_evidence import (
    ...
    select_authoritative_prior_production_evidence,
    validate_prior_production_ok_bindings,
)
...
_BUCKET_RE = re.compile(r"^dealbrain-production-release-artifacts-[0-9]{12}$")
...
def validate_production_identity(
    *,
    ...
    if "staging" in ec2_instance_id.lower():
        raise RollbackEligibilityError("staging identifier in ec2_instance_id")
    if not _BUCKET_RE.fullmatch(bundle_bucket):
        raise RollbackEligibilityError(
            f"bundle bucket must match dealbrain-production-release-artifacts-<account>, "
            f"got {bundle_bucket!r}"
        )
    expected_bucket = f"dealbrain-production-release-artifacts-{expected_account_id}"
    ...
    if "staging" in repo:
        raise RollbackEligibilityError("staging image repository forbidden")
...
def validate_prior_production_approval(prior_deploy_evidence: dict) -> None:
    """Require a prior successful production_ok evidence for the target release."""
    if prior_deploy_evidence.get("final_status") != "production_ok":
        raise RollbackEligibilityError(
            "target release was not previously deployed/approved for production "
    ...
    try:
        ...
        try:
            if args.prior_candidates_dir is not None:
                ...
                prior = select_authoritative_prior_production_evidence(
                    pairs,
...
__all__ = [
    ...
    "validate_prior_production_approval",
    "validate_prior_production_ok_bindings",
    "validate_production_identity",
    "validate_target_differs_from_current",
    "validate_target_manifest_authority",
