#!/usr/bin/env python3
"""Accept authoritative host production rollback evidence — never synthesize success.

Missing evidence fails closed. This script refuses to create rollback_ok.

Invoke from the repository root as a module:

  python -m scripts.deploy.write_gha_production_rollback_evidence [options]
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to host-authored production-rollback-evidence.json (must already exist)",
    )
