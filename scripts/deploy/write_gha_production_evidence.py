#!/usr/bin/env python3
"""Accept authoritative host production evidence — never synthesize success.
...
Missing evidence fails closed. This script refuses to create production_ok.

Invoke from the repository root as a module so ``scripts.*`` imports resolve:

  python -m scripts.deploy.write_gha_production_evidence [options]
"""
...
from scripts.deploy.production_evidence import EvidenceError, load_evidence, validate_evidence_bindings
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to host-authored production-deploy-evidence.json (must already exist)",
    )
    ...
    if not args.evidence.is_file():
        print(
            ...
            "(refusing to fabricate production_ok)",
            file=sys.stderr,
        )
        return 1
