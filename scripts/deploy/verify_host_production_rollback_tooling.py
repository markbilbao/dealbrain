#!/usr/bin/env python3
"""Verify production host rollback tooling capability before mutation.
...
execute arbitrary commands or touch secrets/staging.
"""
...
EXPECTED_CAPABILITY = "production-rollback"
EXPECTED_TOOLING_VERSION = "ea-p1"
SHA256_HEX_RE = re.compile(r"^[0-9a-f]{64}$")

REQUIRED_TOOLING_FILES = (
    "dealbrain-production-rollback.sh",
    "deploy_atomicity.sh",
    "production_rollback_evidence.py",
    "write-production-rollback-evidence.py",
    "prior_production_evidence.py",
    "resolve-production-rollback-migration.py",
    "verify_host_production_rollback_tooling.py",
    "production-rollback-evidence.schema.json",
    ...
    *,
    ...
    if not capability_path.is_file():
        raise HostToolingError(
            ...
            "(Deploy Production must refresh host tooling before rollback)"
        )
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Defaults to <bin-dir>/production-host-tooling.json",
    )
    parser.add_argument(
    ...
    capability_path = args.capability_path or (args.bin_dir / "production-host-tooling.json")
    try:
        if args.write:
