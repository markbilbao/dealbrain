"""Early Access production deploy evidence creation and integrity validation.
...
def resolve_schema_path(module_file: Path | None = None) -> Path:
    ...
    candidates = (
        here / "production-deploy-evidence.schema.json",
        here.parents[1] / "schemas" / "production-deploy-evidence.schema.json",
    )
...
ALLOWED_STATUSES: Final[frozenset[str]] = frozenset({"production_ok", "failed"})
...
# Exact production-deploy-evidence.schema.json contract (stdlib host fallback).
# Keep in lockstep with schemas/production-deploy-evidence.schema.json — drift fails closed.
...
def _require_nonempty_str(payload: dict[str, Any], key: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise EvidenceError(f"{key} must be a non-empty string for production_ok")
    return value


def _validate_production_ok_semantics(payload: dict[str, Any]) -> None:
    reason = payload.get("failure_reason")
    if reason not in (None, ""):
        raise EvidenceError("production_ok requires failure_reason null or empty")

    after = payload.get("migration_revision_after")
    if not isinstance(after, str) or not after.strip():
        raise EvidenceError("production_ok requires non-empty migration_revision_after")
    if not MIGRATION_REV_RE.fullmatch(after.strip()):
        raise EvidenceError("production_ok migration_revision_after failed migration contract")
    ...
    if (
        ...
        raise EvidenceError("production_ok migration_revision_before failed migration contract")

    for gate in ("localhost_live", "localhost_ready", "alb_target_healthy", "smoke_ok"):
        if payload.get(gate) is not True:
            raise EvidenceError(f"production_ok requires {gate} == true")

    image_id = _require_nonempty_str(payload, "image_id")
    if not image_id.startswith("sha256:"):
        raise EvidenceError("production_ok image_id must be a sha256 digest id")
    ...
    if not repo_digest.endswith(expected_suffix) and image_digest not in repo_digest:
        raise EvidenceError("production_ok repo_digest must match image_digest authority")
    ...
    if not TIMESTAMP_RE.fullmatch(created) and not UTC_Z_RE.fullmatch(created):
        raise EvidenceError("production_ok image_created_at must be a valid timestamp")
    ...
    if not UTC_Z_RE.fullmatch(started):
        raise EvidenceError("production_ok deployment_started_at must be UTC Z")
    if not UTC_Z_RE.fullmatch(finished):
        raise EvidenceError("production_ok deployment_finished_at must be UTC Z")

    duration = payload.get("deployment_duration_seconds")
    if not isinstance(duration, int) or duration < 0:
        raise EvidenceError("production_ok deployment_duration_seconds must be >= 0")
    ...
    if not INSTANCE_ID_RE.fullmatch(instance_id):
        raise EvidenceError("production_ok requires a valid EC2 instance id")

    ssm_command_id = payload.get("ssm_command_id")
    if not isinstance(ssm_command_id, str) or not ssm_command_id.strip():
        raise EvidenceError("production_ok requires non-empty ssm_command_id")
    ...
    if not ACCOUNT_RE.fullmatch(account):
        raise EvidenceError("production_ok requires a 12-digit aws_account_id")
...
def validate_evidence(payload: dict[str, Any]) -> None:
    ...
    if status == "production_ok":
        _validate_production_ok_semantics(payload)
    elif status == "failed":
...
def validate_evidence_bindings(
    ...
    *,
    ...
    ec2_instance_id: str,
    ssm_command_id: str,
    require_production_ok: bool = True,
) -> None:
    ...
    if require_production_ok and payload.get("final_status") != "production_ok":
        raise EvidenceError(
            f"authoritative evidence final_status must be production_ok, "
