...
- **2026-09-23 Shopify access-stage reconciliation:** The documented Anonymous Global Catalog catalog mode does not require a separate application, separate provider preapproval, or merchant/API credentials. An agent profile is required. Staging profile and staging technical connection are validated. Production profile remains undeployed. Production provider, executable certification, and routing remain absent. Production certified remains false. EXT-01 stays `applied`. EXT-06 stays `not_started` and is not applicable to that Anonymous path. Sprint 32 remains open. Sprint 38 and Sprint 41 remain unstarted. This is not production ready.
...
5. **Sprint 31 owner close** — satisfied. Router/provider contract is merged; unification ADR, September 15 dual-path disposition (recorded 2026-09-05: retain intentional dual implementations), and onboarding runbook are recorded. Sprint 31 remains formally closed. Sprint 32 has started and remains open. It is not blocked on a Shopify application, preapproval, or credential stage that the documented Anonymous catalog mode does not require. Production certification is still incomplete.
6. **Minimum viable market certification (32–36)** — at least one legally usable real source path whose actual access requirements are satisfied, with current-data and capability-policy evidence. Credential approval is required only where the selected path requires credentials. Others may be omitted. Sprint 32 foundation is implemented. Shopify Anonymous catalog access stages are reconciled and are not production certification. The executable PH production path is still missing.
