...
| 37 | MarketContext, Currency & Localization | Coherent market/FX/locale + shipping honesty + destination re-evaluation | P1-1B; P1-2 | EXT-23 | 37.1–37.3 merged; 37.4 destination-change fail-closed / re-evaluation readiness implemented. Live evidence-backed destination re-evaluation remains **unavailable**. `DESTINATION_REEVALUATION_IMPLEMENTED` remains **False**. Production FX conversion remains **unavailable**. Production certified shopping markets remain **zero**. EXT-23 stays `not_started`. Sprint 38 is **not started**. Sprint 37 is **not complete**. Exit still requires a trusted FX source, five-market/unsupported-market completion, shipping honesty closure, and live server-side re-evaluation when destination materially changes cost; FR-CA decision recorded |
