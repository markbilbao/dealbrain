...
**Owner lock date:** 2026-08-24 (launch date / sprint lock); **2026-09-06** (PH beta merchant neutrality + effective-cost launch lock); **2026-09-07** (public beta without affiliate monetization + PH validation-beta Sprint 26 reconciliation); **2026-09-08** (EXT-01 `applied` + Sprint 26 COMPLETE / CLOSED)
...
**Sprint 26 technical evidence:** [`evidence/SPRINT_26_STAGING_CURRENT_MAIN_PROOF.md`](evidence/SPRINT_26_STAGING_CURRENT_MAIN_PROOF.md) — current-main staging proof verified for SHA `79bd03f`; Sprint 26 **COMPLETE / CLOSED** 2026-09-08 after EXT-01 `applied` ([`evidence/SPRINT_26_COMPLETION.md`](evidence/SPRINT_26_COMPLETION.md))
...
> **OWNER ROADMAP LOCK — 2026-09-06**
>
> September 30, 2026 remains the owner target for Controlled Public Beta. Do not move or soften that date. Date pressure reduces optional scope; it does not weaken truthfulness, legal, privacy, security, merchant-certification, production, or evidence gates.
>
> Initial commercial/product validation focus is **Philippines first**. That is a market-priority decision, not a restriction that PiqSavi may only ever search two marketplaces.
>
> For the September PH beta, the initial affiliate-monetization targets are **Shopee Philippines** and **Lazada Philippines**. They are monetization targets, not ranking privileges and not the exclusive search/recommendation universe.
>
> Permanent product rule: **affiliate economics are downstream of the shopping decision.** Affiliate status must never exclude an otherwise legitimate source from consideration. All relevant, enabled, certified merchant/data sources for the shopper's market are eligible for server-side research routing regardless of monetization status. A non-affiliate merchant that is actually evaluated may receive canonical PiqScore, become Best Piq, become the Recommendation, rank above affiliate merchants, and receive a normal outbound merchant link. If a non-affiliate merchant wins, PiqSavi may earn ₱0 and must still recommend it. This does **not** mean every integrated merchant must be queried on every shopper request. The research/router may decide which sources are actually attempted using legitimate non-affiliate factors (shopper market, requested category/product, connector capability, provider restrictions, coverage, source health, availability, timeout/degradation, and other operational relevance). Affiliate commission, affiliate availability, or partner economics must never include/exclude a source from organic consideration, prioritize a source, or alter PiqScore, Recommendation, Best Piq, or organic ordering.
>
> Search inclusion depends on a **legitimate, sufficiently trustworthy data path**, not affiliate status. Do not claim universal marketplace or whole-internet coverage. Do not use scraping as a workaround for unavailable rights. Public language remains scoped to **Best Piq among the offers PiqSavi evaluated**. Do not imply all supported merchants were queried unless execution evidence proves they were. Sprint 38 execution traces remain responsible later for truthful attempted / succeeded / failed / timed-out sources and evaluated-offer count; this lock does not start Sprint 38.
>
...
>
> September beta ranking must use the **best verified effective purchase cost** available from certified evidence — not sticker/listing price alone when reliable purchase-cost components exist. Reuse the existing Sprint 29 canonical offer-economics architecture. Do not create a second price model. Do not silently rewrite PiqScore ownership.
>
...
- Affiliate availability and commission amount must never include/exclude a source from organic consideration, prioritize a source, or alter PiqScore, Recommendation, Best Piq, organic ordering, or which merchant wins.
- Search/recommendation coverage is merchant-neutral: all relevant, enabled, certified sources for the shopper's market remain eligible for routing regardless of monetization, including official brand stores, direct retailers, electronics retailers, authorized reseller sites, and other marketplaces where a legitimate data path exists. Eligibility is not a requirement to query every integrated merchant on every request. The router may select actually attempted sources using legitimate non-affiliate operational factors.
- Search inclusion depends on legitimate data access (official API, authorized product feed, approved affiliate/product feed, direct retailer/partner integration, permitted public data source, or another contractually/technically legitimate source). Affiliate status is not the inclusion or exclusion test. Scraping is not a workaround for unavailable rights. Public wording remains Best Piq among the offers PiqSavi evaluated.
- Upstream payload presence is not permission to expose or use data. Technical field exposure ≠ policy authorization ≠ offer/shopper applicability.
...
| 26 | Staging Current-Main Proof & Roadmap Bootstrap | **COMPLETE / CLOSED** — technical proof packaged; EXT-01 `applied` 2026-09-08 |
| 27–28 | Identity/email; privacy/legal | 27 COMPLETE / CLOSED. 28 INTERNAL ENGINEERING + STAGING VERIFICATION COMPLETE — EXTERNAL LEGAL/PUBLICATION GATES REMAIN |
| 29 | Production Consumer Decision Experience & Conversational Continuity | **INTERNAL CONSUMER/CONVERSATIONAL CONTRACT COMPLETE — LIVE RESEARCH ACCEPTANCE REMAINS DEPENDENT** — see §5.1 |
...
|--------|------|-----------------|-------------------------|-----------------------|-----------|
| 26 | Staging Current-Main Proof & Roadmap Bootstrap | Historical launch-candidate staging-proven (`79bd03f`); EXT-01 PH product-data request `applied` 2026-09-08 | P0-6 | EXT-01 `applied` (not approved); EXT-08,10,17,18 done for Sprint 26 | **COMPLETE / CLOSED.** Technical: Staging `/ready` + smoke on evidenced digest ([evidence](evidence/SPRINT_26_STAGING_CURRENT_MAIN_PROOF.md)). EXT-01 request evidence: [`evidence/EXT-01_PH_PRODUCT_DATA_ACCESS_REQUESTS_2026-09-08.md`](evidence/EXT-01_PH_PRODUCT_DATA_ACCESS_REQUESTS_2026-09-08.md). Close record: [`evidence/SPRINT_26_COMPLETION.md`](evidence/SPRINT_26_COMPLETION.md). EXT-02…05/`07` are `n_a_beta`. Later SHA `ab23d29` is **not** a Sprint 26 close. Sprint 32 remains pending certification. |
| 27 | Transactional Identity & Email | Real email; reset/verify/email-change; token lifecycle; enumeration-safe errors; session rotation | P0-5; HIGH demo-auth | EXT-08, EXT-09 | **COMPLETE / CLOSED.** 27.1–27.4 implemented. 2026-09-08 staging inbox E2E passed; auth-aware header/logout passed; EXT-09 **PASS / VERIFIED**. PR #121 merged at `a5468ecf65be40bb36a053a97869cec97e3a529c`; Deploy Staging #32 succeeded; live `/health` confirmed `identity_email_adapter=resend` / `identity_email_ready=true`. Production secret attach remains Sprint 41. Sprint 27 completion does **not** mean production transactional email is live. Evidence: [`evidence/SPRINT_27_STAGING_EMAIL_EVIDENCE_2026-09-08.md`](evidence/SPRINT_27_STAGING_EMAIL_EVIDENCE_2026-09-08.md) |
| 28 | Privacy, Legal, Consent & Deletion | ToS/Privacy/consent/deletion/export + search-index privacy | P0-4; MEDIUM GDPR | EXT-17…22 | **INTERNAL ENGINEERING + STAGING VERIFICATION COMPLETE — EXTERNAL LEGAL/PUBLICATION GATES REMAIN.** Not COMPLETE/CLOSED. 28.1 foundations + 28.2 staging export/delete HTTP evidence remain. 2026-09-10 owner browser verification of fail-closed `/privacy` `/terms` `/health` `/support` `/account#consents` and publication-status is recorded for SHA `fc8be5fe2abb0c73e5db7389ce41c4d348f4ffa0`. Counsel drafts remain unpublished. EXT-19 written **conditional** approval recorded 2026-08-19 ([`evidence/EXT-19_COUNSEL_APPROVAL_RECORD_2026-08-19.md`](evidence/EXT-19_COUNSEL_APPROVAL_RECORD_2026-08-19.md)); written unconditional published-scope approval absent; status remains `applied`. EXT-20/21 `not_started`. EXT-22 `not_started` (Sprint 39 owns CMP/analytics activation). 44/45 publication remain open. Evidence: [`evidence/SPRINT_28_INTERNAL_STAGING_VERIFICATION_2026-09-10.md`](evidence/SPRINT_28_INTERNAL_STAGING_VERIFICATION_2026-09-10.md); Early Access re-audit [`evidence/EARLY_ACCESS_LEGAL_RECONCILIATION_AND_LAUNCH_READINESS_2026-09-10.md`](evidence/EARLY_ACCESS_LEGAL_RECONCILIATION_AND_LAUNCH_READINESS_2026-09-10.md) |
...
| 32 | Philippines Merchant Certification | ≥1 real PH **product-data** path. Initial and only September validation market unless owner expands. Record effective-cost field evidence (technical exposure ≠ policy ≠ applicability). TikTok not launch-critical. Affiliate tracking not required. Access stages follow the selected path: approval and credentials where that path requires them; no invented application or credential stage for a documented keyless mode. | P0-1 (PH) | EXT-01; EXT-06 only when the selected path requires credentials | Real legally usable current-data response + capability-policy evidence plus distinct technical field-exposure records; staging+limited proof. Fixtures cannot certify. Affiliate-only paths cannot independently satisfy EC-09. Shopee/Lazada are not certified by this lock. EXT-06 is not applicable to documented Anonymous Shopify Global Catalog access. |
...
| 37 | MarketContext, Currency & Localization | Coherent market/FX/locale + shipping honesty + destination-change fail-closed contract for the PH-only beta | P1-1B; P1-2 | EXT-23 optional; stays `not_started` | **COMPLETE / CLOSED** (2026-09-26) for the PH-only public beta. P1-1B and P1-2 are closed for this scope. US/SG/UK/CA and FR-CA are not applicable. Production FX stays unavailable. `DESTINATION_REEVALUATION_IMPLEMENTED` stays **False**. Live evidence-backed re-evaluation is Sprint 38. Certified shopping markets stay **zero**. Close record: [`evidence/SPRINT_37_COMPLETION.md`](evidence/SPRINT_37_COMPLETION.md). Not launch ready. |
| 38 | Connector Reliability & Honest Degradation | Production live-research execution + truthful degradation | Live-HTTP risk | EXT-25 | **IN PROGRESS** (2026-09-26). Engineering foundation only. Not COMPLETE / CLOSED. Live mode stays fail-closed. Routing stays 0. No Shopify call. Multi-connector live chaos, probes, and production alerts remain future scope or Sprint 42. |
...
| Remaining internal consumer/conversational contract | **documented 2026-09-15** | Current-`main` audit baseline `3c51494` (PR #141 merge). Verdict **INTERNAL CONSUMER/CONVERSATIONAL CONTRACT COMPLETE — LIVE RESEARCH ACCEPTANCE REMAINS DEPENDENT**. Evidence: [`evidence/SPRINT_29_CURRENT_MAIN_RECONCILIATION_2026-09-15.md`](evidence/SPRINT_29_CURRENT_MAIN_RECONCILIATION_2026-09-15.md). Stale PR #127 is superseded and must not be merged as-is. |
| Final checkpoint | **documented 2026-09-18** | Baseline `4a4fe65` (PR #142 merge). Reconfirms the same verdict. **Not** COMPLETE/CLOSED. No additional Sprint-29-owned implementation. Remaining CC-01-10/11/12/24 acceptance still requires later-sprint evidence. Evidence: [`evidence/SPRINT_29_FINAL_CHECKPOINT_2026-09-18.md`](evidence/SPRINT_29_FINAL_CHECKPOINT_2026-09-18.md). |
...
**Do not** mark Sprint 29 closed. Internal contract completeness does not certify live merchant research, MarketContext policy, support backends, visual sign-off, or the immutable launch-candidate CC-01 journey. Early Access being live is **not** public shopping beta.
...
|-------------------|-------------------|-------------------|---------------------|-----------------|
| Current-main staging proof | Older SHA staging proven; **`79bd03f` staging_ok packaged**; later SHAs including `d62a6fb` and PR #114 `1f66688` are not Sprint 26 close evidence | 26 | Evidence JSON + `/ready` on evidenced digest — see [`evidence/SPRINT_26_STAGING_CURRENT_MAIN_PROOF.md`](evidence/SPRINT_26_STAGING_CURRENT_MAIN_PROOF.md); Sprint 26 **COMPLETE / CLOSED** 2026-09-08 (EXT-01 `applied`); later SHAs need their own staging proof before launch | Yes |
...
| ToS / Privacy / consent | Missing published URLs | 28 | Published URLs + consent records (engineering gate + inspection exist; catalog unpublished) | Yes — NON-WAIVABLE |
| Account deletion / export | Engineering APIs + 28.2 staging HTTP | 28 | Delete+export E2E; propagation checklist — 28.2 HTTP recorded; not legal DSAR | Yes — NON-WAIVABLE |
...
|--------|-------------------------|-------------------------|----------------------|-------------|
| Philippines | ≥1 real legally usable connector/feed/legitimate product-data path. Initial and only September supported-market target unless owner expands. Affiliate monetization is **not** a September launch requirement. Shopee PH + Lazada PH remain eligible product-data candidates, not affiliate launch blockers. | PHP + EN; MarketContext | 32 | A legitimate authorized PH access path whose actual requirements are satisfied, plus remaining certification evidence, staging+prod validation, and a published coverage row. EXT-01 Shopee/Lazada remain `applied`, not provisioned. Documented Anonymous Shopify Global Catalog does not require EXT-06 credentials. TikTok Shop PH is not launch-critical. EXT-07 affiliate tracking is not a launch blocker. |
...
| Canada | ≥1 real path if later named | CAD + EN if later named. Sprint 37 PH-only closure: FR-CA is not applicable because Canada is omitted | 36 if later named | EXT-05 is `n_a_beta` for September |
...
|------------|-------|---------------|----------------|--------------|----------|---------------|
| Merchant approvals PH | Marketplace + legal | 32 | EXT-01 `applied` (2026-09-08; not approved) | **RED** for naming PH; **RED** for September shopping launch until ≥1 useful PH product-data path is certified | Delay PH shopping claims | PH product-data path — NON-WAIVABLE for September shopping beta |
...
| EC-26 | A destination change that could materially change shipping or effective cost triggers the server-authoritative re-evaluation requirement. While no live evidence-backed executor exists, the state is `required_unavailable` and the prior canonical decision, PiqScore, and Recommendation remain unchanged | 37 owns the fail-closed contract; live execution is Sprint 38 | PH-only staging proof of `required_unavailable` and no client-side fake repricing. Not proof of a live executor | No-go if the UI invents new prices or Best Piq. `DESTINATION_REEVALUATION_IMPLEMENTED` remains False |
...
**Market note for EC-09:** When multiple markets are named, each named market’s certification sprint (32/33/34/35/36) is the primary owner for that market’s path. Sprint 31 owns the shared capability/policy contract and fail-closed harness; 32–36 populate provider-specific evidence. Sprint 44/45 verify and may remove markets or providers. A named market needs a legitimate authorized access path whose actual access requirements are satisfied. That is provider approval, provisioning, and credentials where the selected path requires them, or official documentation of a public/keyless/anonymous mode with no separate credential or preapproval requirement, plus the remaining certification evidence. The keyless exception applies only to that documented path and does not weaken Shopee, Lazada, or other credential-required providers.

**EC-09 capability-policy invariant:** No merchant connector may be certified for production, or used to support a named shopping-market claim, unless its permitted affiliate, data-use, caching, transformation, comparison, attribution, and related capabilities are explicitly declared, evidence-backed, and fail-closed enforced. Unknown permissions do not enable production features. Provider approval alone does not imply blanket capability approval. Affiliate-only destinations cannot independently satisfy EC-09 current-data market naming. PH (and later market) certification must also record effective-cost components — current listing price, seller/platform discount, voucher/promotion information, voucher eligibility/applicability information, destination-dependent shipping, free-shipping status, unavoidable checkout/other costs where exposed, and timestamp/freshness — as three distinct facts: technical/source field exposure; Sprint 31 policy authorization (`allowed` / `restricted` / `prohibited` / `unknown`); and offer/shopper applicability where relevant. Do not treat policy state as technical availability. Do not invent a parallel authorization model.
...
### 9.5 OWNER LAUNCH-QUALITY LOCK — effective purchase cost (2026-09-06)

September beta recommendation must not rank offers using sticker/listing price alone when reliable purchase-cost components are available. The launch experience evaluates the **best verified effective purchase cost** available from certified evidence.

Conceptual composition (reuse Sprint 29 canonical offer economics; do **not** create a second price model):

effective purchase cost
= listing price
− verified currently applicable discounts
− verified currently applicable vouchers/promotions
+ known destination-relevant shipping
+ known unavoidable taxes/duties/import/checkout costs

Sprint 29 already owns/preserves listing price, verified discounts, verified vouchers where captured, shipping, taxes/duties/import costs, dominant price state, structured unknowns, and provenance/freshness. Sprint 37 owns shipping/destination honesty and destination-sensitive re-evaluation. This lock does **not** silently rewrite PiqScore ownership.

**Hard truthfulness — discounts and vouchers.** A discount, voucher, promo, coin equivalent, seller coupon, platform coupon, or similar reduction may affect effective cost / PiqScore only when **all** of these are true: the field is technically exposed by the authorized path; Sprint 31 policy permits its use; and evidence supports applicability to the evaluated offer under the known shopper context. Field present ≠ permitted to use. Permitted to use ≠ field actually available. Voucher available ≠ voucher applicable to this shopper/offer. Insufficient evidence includes: generic “up to ₱500 off” banners; a voucher that exists somewhere on the marketplace with unknown offer eligibility; new-user-only discounts with unknown user eligibility; payment-method discounts with unknown payment method; minimum-spend vouchers whose threshold/applicability is not established; campaign discounts inferred only because 9.9 / 11.11 is approaching. Such values may be shown as potential/conditional savings if source rights and evidence allow; they must not be silently subtracted from scored effective cost.

**Hard truthfulness — shipping.** Unknown shipping must never become ₱0, FREE, included, or assumed negligible unless evidence supports that state. If destination-specific shipping is known, it participates in effective purchase cost. If shipping materially depends on shopper destination and the shopper supplies or changes destination, use Sprint 37's server-side re-evaluation contract. No client-side fake repricing.

**Current Sprint 37 closure (2026-09-26).** That contract is `required_unavailable` while no live evidence-backed executor exists. The prior canonical decision, PiqScore, and Recommendation stay unchanged. Live evidence-backed re-evaluation remains Sprint 38. `DESTINATION_REEVALUATION_IMPLEMENTED` remains False. This note does not rewrite the 2026-09-06 lock above.
...
|--------|---------|--------------------------------|----------------|-----------|
| Shopee | ₱18,500 | verified voucher −₱500; shipping ₱150 | ₱18,150 | available |
| Lazada | ₱18,250 | verified free shipping | ₱18,250 | available |
| Official retailer | ₱18,000 | verified free shipping; local warranty evidence | ₱18,000 | none |

If the official retailer is strongest under the objective scoring model, it may become Best Piq even though PiqSavi earns nothing.

**EC-31 evidence on the frozen launch candidate.** Where certified sources expose these fields, a real or suitably certified staging case must demonstrate that:

1. a verified discount can reduce effective cost;
...
This is a clarification/strengthening of existing canonical offer economics and Sprint 37 shipping honesty. It is **not** Sprint 47 implementation. Do not pull historical campaign prediction, promotion timing intelligence, Buy Now / Wait / Watch, future-sale prediction, campaign-aware Wait, or price-drop monitoring into September.
...
| SEO foundation / private noindex | Yes (Sprint 29 foundation) | Yes | Partial (`a8bd001` noindex); GSC pending | Pending (45) | Pending (45) |
...
| Searches stores worldwide | ≥1 real path in each named market | 32–36 / 44 | “Searches supported merchants in [list]” / “Best Piq among the offers PiqSavi evaluated” | “Searches stores worldwide”; implying TikTok or any unqueried marketplace was searched; implying all supported merchants were queried without execution evidence; implying Shopee + Lazada are the only merchants considered |
| Compares all major retailers | Never for beta | — | — | Any “all major retailers” |
| Shopee / Lazada live, approved, production-ready, or contractually usable | Sprint 32 production certification + claims approval | 32 / 44 / 45 | None until actual certification is complete | Any live/certified/approved claim before evidence |
| TikTok Shop PH searched / supported | Actual supported query path + claims approval | 32 / 44 / 45 | None for September unless actually supported | Naming TikTok in marketplace-coverage claims without a real path |
...
- A legitimate authorized access path whose actual access requirements are satisfied: provider approval, provisioning, and credentials where the selected path requires them, or official documentation of a public/keyless/anonymous mode with no separate credential or preapproval requirement
- Remaining certification evidence, including a real provider response in staging and production (or production-equivalent rehearsal)
- Legal/terms evidence recorded for the selected path, without inventing an approval stage the provider does not document
...
The 2026-09-06 effective-purchase-cost launch lock is a clarification/strengthening of existing Sprint 29 canonical offer economics and Sprint 37 shipping honesty. It is **not** Sprint 47 implementation and does **not** pull these advanced items into September:

- historical campaign prediction
- promotion timing intelligence
- Buy Now / Wait / Watch layer
- future-sale prediction
- campaign-aware Wait
- price-drop monitoring
- Sprint 47.1–47.5 promotion-evidence / effective-purchase-price / historical-price / buying-action / Watch-integration work
...
| 21 | Privacy/consent | 28 / 39 | Internal inspection + 28.1 persistence implemented; 2026-09-10 owner verified unpublished empty acknowledgements; production records stay empty | Consent persistence + staging proof after publication | Yes — NON-WAIVABLE | Delay launch |
| 22 | Deletion/export | 28 | 28.1 APIs implemented; 28.2 staging HTTP evidence recorded | Staging E2E recorded (engineering, not legal DSAR) | Yes — NON-WAIVABLE | Delay launch |
...
| 28 | Affiliate neutrality/disclosure | 20 / 28 / 40 / 44 | Engine verified; legal pending; launch monetization deferred | Neutrality tests + future-capable legal disclosure; canonical UUID pages omit inactive affiliate disclosure | Yes — NON-WAIVABLE | Ordinary outbound links; future disclosure attachment is not enabled by this lock |
...
| 43 | Launch go/no-go | 45 | Planned | EC-01…EC-31 + checklist | Yes — NON-WAIVABLE | No-go or rollback |
...
- Sprint 32 **COMPLETE / CLOSED** (2026-09-25) for the reduced Shopify Global Catalog Anonymous path only. Not production-deployment ready. Not launch ready. Provider `ph-shopify-global-catalog` is registered and operationally DISABLED. Evidence rows = 4. Trusted certifications = 4. Routing = 0. Public certified shopping markets = 0. Live research execution remains unimplemented (Sprint 38). Production UCP profile remains undeployed (Sprint 41). Close record: [`evidence/SPRINT_32_COMPLETION.md`](evidence/SPRINT_32_COMPLETION.md).
- Sprint 26 **COMPLETE / CLOSED** (2026-09-08). Technical proof remains `79bd03f`, not the current baseline. Shopee Sprint 26 affiliate evidence records affiliate dashboard access; Payment & Tax submitted/pending; Affiliate Open API documented but **not granted**. See [`evidence/SPRINT_26_SHOPEE_APPLICATION_EXECUTION.md`](evidence/SPRINT_26_SHOPEE_APPLICATION_EXECUTION.md). That affiliate record is **not** Sprint 32 certification and is **not** EXT-01 approval. Separate 2026-09-08 product-data request emails moved EXT-01 to `applied`.
- EXT-01 PH product-data access **`applied`** (2026-09-08). Not `approved` / not `provisioned`. EXT-02…EXT-05 and EXT-07 are `n_a_beta` for this beta. EXT-06 remains `not_started` for credential-required paths and is **not applicable** to the documented S-1 Anonymous Shopify Global Catalog path.
...
- **2026-09-07 lock (historical for Sprint 26 open/EXT-01 `not_started`):** Public beta launches without affiliate monetization. Product validation is the launch objective. Ordinary outbound merchant links are valid launch behavior. Affiliate architecture is preserved for later activation. Shopee/Lazada affiliate approval is not a launch dependency. Product-data certification remains a launch dependency. EC-31 mixed-affiliate runtime proof is conditional. September 30 target is unchanged. This lock is primarily roadmap/policy reconciliation plus a narrow launch-UI honesty change that removes inactive affiliate-disclosure copy from canonical UUID pages; it does not enable a future affiliate-disclosure attachment path. Additive Sprint 26 / PH-only reconciliation as of 2026-09-07: September supported-market target is Philippines only; EXT-01 remains legitimate PH product-data access (`not_started` on that date); EXT-02…05 and EXT-07 are `n_a_beta`; Sprint 26 remained open for EXT-01 only.
- **2026-09-08 Sprint 26 close:** EXT-01 `not_started` → `applied` on owner-supplied Lazada PH and Shopee PH product-data request evidence. Sprint 26 is **COMPLETE / CLOSED**. See [`evidence/EXT-01_PH_PRODUCT_DATA_ACCESS_REQUESTS_2026-09-08.md`](evidence/EXT-01_PH_PRODUCT_DATA_ACCESS_REQUESTS_2026-09-08.md) and [`evidence/SPRINT_26_COMPLETION.md`](evidence/SPRINT_26_COMPLETION.md). EXT-01 is not approved/provisioned. Sprint 32 remains pending certification. Affiliate monetization remains deferred.
- **2026-09-23 Shopify access-stage reconciliation:** The documented Anonymous Global Catalog catalog mode does not require a separate application, separate provider preapproval, or merchant/API credentials. An agent profile is required. Staging profile and staging technical connection are validated. Production profile remains undeployed. The 2026-09-23 wording that production provider and certification were absent is historical for that date. Current production state is one disabled provider, four evidence rows, four trusted reduced-capability certifications, and zero routing policies. Executable production certification remains absent. Production certified remains false. EXT-01 stays `applied`. EXT-06 stays `not_started` and is not applicable to that Anonymous path. Sprint 32 is COMPLETE / CLOSED for the reduced path only. Sprint 38 and Sprint 41 remain unstarted. This is not production ready.
- **2026-09-25 Sprint 32 closure:** SPRINT 32 STAGING CERTIFICATION = PASSED. REDUCED CAPABILITY SET ONLY. NOT PRODUCTION DEPLOYMENT READY. Owner normalization attempt #3 is final. Engineering kill-switch validation passed on the real descriptor in memory. DEPLOYED OPERATIONAL KILL-SWITCH DRILL remains Sprint 38/41. Public PH shopping coverage is not live. Sprint 38 is unstarted. Sprint 41 is unstarted.

### Strict gates (cannot be skipped)

1. **Sprint 29 remaining launch proof** — internal consumer/conversational contract is complete on current `main` (`4a4fe65`). Remaining Sprint 29 **acceptance** is the frozen-candidate CC-01 staging journey after live research exists. SEO technical foundation and persistent Ask are merged. Live research remains Sprints 31–38. Early Access legal pages, production deploy path, and production transactional email are **not** Sprint 29 implementation gaps. Sprint 29 is **not** closed.
...
5. **Sprint 31 owner close** — satisfied. Router/provider contract is merged; unification ADR, September 15 dual-path disposition (recorded 2026-09-05: retain intentional dual implementations), and onboarding runbook are recorded. Sprint 31 remains formally closed. Sprint 32 is COMPLETE / CLOSED for the reduced Shopify path and is not production-deployment ready. It was not blocked on a Shopify application, preapproval, or credential stage that the documented Anonymous catalog mode does not require. Production deployment certification remains a later-sprint gate.
6. **Minimum viable market certification (32–36)** — at least one legally usable real source path whose actual access requirements are satisfied, with current-data and capability-policy evidence. Credential approval is required only where the selected path requires credentials. Others may be omitted. Sprint 32 is COMPLETE / CLOSED for the reduced Shopify Global Catalog Anonymous path. That closure is not production deployment, live execution, or public market activation. The executable PH production path remains Sprint 38 and Sprint 41.
7. **Sprint 37** — **COMPLETE / CLOSED** (2026-09-26) for the PH-only public beta. Market/currency/locale honesty, unsupported-market fail-closed behavior, and shipping honesty are in place. Production FX stays unavailable. Live destination re-evaluation stays Sprint 38. Public PH shopping coverage stays disabled.
8. **Sprint 38** — **IN PROGRESS** (2026-09-26). Engineering foundation for execution identity, fail-closed live mode, truthful traces, and honest degradation. The Ask PiqSavi authorization and Sprint 31 plan now hand off into non-live preparation. Not complete. Live execution remains NOT OPERATIONAL. No Shopify call. Routing remains 0. Public PH shopping coverage remains disabled. Sprint 41 remains UNSTARTED.
...
See §8 and the register. The items that can miss September 30 unless resolved or scope-reduced are currently **RED/AMBER**: PH product-data access (RED until ≥1 certified PH market), counsel written approval + policy publication (AMBER), production DNS/TLS/AWS/secrets (AMBER), paging (AMBER), Search Console (AMBER), FX (AMBER, reducible), AI quota (AMBER, reducible). EXT-09 sender-domain auth is **GREEN** for the Sprint 27 evidence claim (Resend **Verified** + public DNS, 2026-09-08; not production email live). EXT-02…05 are **GREEN** for September (`n_a_beta`). EXT-07 affiliate tracking is optional, `n_a_beta`, and is **not** a September launch blocker (2026-09-07).
...
| [`evidence/`](evidence/) | Packaged sprint evidence (Sprint 26 current-main staging proof + bootstrap checklist + 2026-09-08 EXT-01 requests + Sprint 26 completion) |
| [`evidence/SPRINT_26_STAGING_CURRENT_MAIN_PROOF.md`](evidence/SPRINT_26_STAGING_CURRENT_MAIN_PROOF.md) | Sprint 26 technical staging proof (historical package; sprint later CLOSED 2026-09-08) |
| [`evidence/SPRINT_26_COMPLETION.md`](evidence/SPRINT_26_COMPLETION.md) | Authoritative Sprint 26 COMPLETE / CLOSED record |
| [`evidence/EXT-01_PH_PRODUCT_DATA_ACCESS_REQUESTS_2026-09-08.md`](evidence/EXT-01_PH_PRODUCT_DATA_ACCESS_REQUESTS_2026-09-08.md) | EXT-01 `applied` evidence (Lazada PH + Shopee PH product-data requests) |
| [`evidence/SPRINT_26_SHOPEE_APPLICATION_EXECUTION.md`](evidence/SPRINT_26_SHOPEE_APPLICATION_EXECUTION.md) | Sprint 26 Shopee affiliate evidence (affiliate onboarding active / Open API access pending; not EXT-01 approval; not certification) |
