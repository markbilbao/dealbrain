...
| 41 | Production Environment & Deploy Path | Isolated prod AWS + deploy/rollback + DNS/TLS | P0-2; HIGH prod path | EXT-10…14 | **PARTIALLY COMPLETE — PRODUCTION LIVE.** Live `/ready` READY after Deploy Production #6 (`3c51494`). Rollback workflow exists but is **not** validated against a real prior `production_ok` release. `www` does not redirect to apex. CDN/WAF decision is proposal-only. IaC alone is still not proof. Not COMPLETE/CLOSED. Evidence: [`evidence/SPRINT_41_CURRENT_MAIN_RECONCILIATION_2026-09-18.md`](evidence/SPRINT_41_CURRENT_MAIN_RECONCILIATION_2026-09-18.md) |
...
| Production deploy/rollback | Yes (deploy) | Yes (workflows + Deploy #6 `production_ok`) | Production `/ready` READY 2026-09-18; staging currently 503 | Rollback **not** validated | Pending (41 remaining + 44) |
...
- EXT-01 PH product-data access **`applied`** (2026-09-08). Not `approved` / not `provisioned`. EXT-02…EXT-05 and EXT-07 are `n_a_beta` for this beta. EXT-06 remains unresolved for PH credentials.
...
1. **Sprint 29 remaining launch proof** — internal consumer/conversational contract is complete on current `main` (`3c51494`). Remaining Sprint 29 launch proof is the frozen-candidate CC-01 staging journey after live research exists. SEO technical foundation and persistent Ask are merged. Live research remains Sprints 31–38. Early Access legal pages, production deploy path, and production transactional email are **not** Sprint 29 implementation gaps. Sprint 29 is **not** closed.
...
9. **Sprint 41** — **PARTIALLY COMPLETE — PRODUCTION LIVE; ROLLBACK VALIDATION, WWW CANONICALIZATION, CDN/WAF OWNER DECISION, STAGING PRESERVATION, AND LIVE IAM SIMULATION REMAIN OPEN.** Isolated production AWS/DB/secrets/IAM/deploy/DNS/TLS/hostname are live (Deploy Production #6). IaC alone is still not proof. Rollback workflow exists but is **not** validated. Not COMPLETE/CLOSED. Evidence: [`evidence/SPRINT_41_CURRENT_MAIN_RECONCILIATION_2026-09-18.md`](evidence/SPRINT_41_CURRENT_MAIN_RECONCILIATION_2026-09-18.md).
...
See §8 and the register. The items that can miss September 30 unless resolved or scope-reduced are currently **RED/AMBER**: PH product-data access (RED until ≥1 certified PH market), counsel written approval + policy publication (AMBER), production rollback validation / www canonicalization / CDN/WAF owner decision (AMBER; production DNS/TLS/AWS/secrets are now provisioned — see 2026-09-18 Sprint 41 reconciliation), paging (AMBER), Search Console (AMBER), FX (AMBER, reducible), AI quota (AMBER, reducible). EXT-09 sender-domain auth is **GREEN** for the Sprint 27 evidence claim (Resend **Verified** + public DNS, 2026-09-08). Production identity email is live (`identity_email_ready=true`, 2026-09-18) without closing Sprint 41. EXT-02…05 are **GREEN** for September (`n_a_beta`). EXT-07 affiliate tracking is optional, `n_a_beta`, and is **not** a September launch blocker (2026-09-07).
