...
| 37 | MarketContext, Currency & Localization | Coherent market/FX/locale + shipping honesty + destination re-evaluation | P1-1B; P1-2 | EXT-23 | 37.1 foundation implemented (composed MarketContext, empty certified-market catalog, invalidation-only). Sprint 37 is **not complete**. Exit still requires fail-closed FX, unsupported-market completion, shipping honesty closure, and server-side re-evaluation when destination materially changes cost; FR-CA decision recorded |
