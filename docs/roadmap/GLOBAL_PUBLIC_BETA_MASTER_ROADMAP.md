...
| 27 | Transactional Identity & Email | Real email; reset/verify/email-change; token lifecycle; enumeration-safe errors; session rotation | P0-5; HIGH demo-auth | EXT-08, EXT-09 | **In progress — not complete.** 27.1–27.4 implemented. 2026-09-08 staging inbox E2E passed; auth-aware header/logout passed; EXT-09 **PASS / VERIFIED**. Production secret attach remains Sprint 41. Sprint 27 stays open while `/health` `identity_email_ready=false`. Remaining action: designed production-code readiness-gate update, then staging deploy and `/health` verification. Evidence: [`evidence/SPRINT_27_STAGING_EMAIL_EVIDENCE_2026-09-08.md`](evidence/SPRINT_27_STAGING_EMAIL_EVIDENCE_2026-09-08.md) |
...
3. **Sprint 27** — real transactional email, sender auth, verify/reset/email-change, token lifecycle, enumeration-safe errors, session rotation, staging E2E, production cutover readiness. Staging inbox E2E and EXT-09 Resend **Verified** passed 2026-09-08. Sprint 27 remains open until the designed `identity_email_ready` production-code gate is updated, deployed to staging, and `/health` reports `identity_email_ready=true`.
