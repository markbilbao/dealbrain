...
>
> Permanent product rule: **affiliate economics are downstream of the shopping decision.** PiqSavi must evaluate every legitimately supported merchant, retailer, marketplace, or product-data source available for the shopper's market, regardless of whether PiqSavi earns affiliate commission from that source. A non-affiliate merchant may enter the evaluated set, receive canonical PiqScore, become Best Piq, become the Recommendation, rank above affiliate merchants, and receive a normal outbound merchant link. If a non-affiliate merchant wins, PiqSavi may earn ₱0 and must still recommend it.
>
> Search inclusion depends on a **legitimate, sufficiently trustworthy data path**, not affiliate status. Do not claim universal marketplace or whole-internet coverage. Do not use scraping as a workaround for unavailable rights. Public language remains scoped to Best Piq among the offers PiqSavi evaluated.
>
...
- Affiliate availability and commission amount must not determine which merchants are searched, which offers are evaluated, PiqScore, Recommendation, Best Piq, organic ordering, research routing, or which merchant wins.
- Search/recommendation coverage is merchant-neutral: every legitimately supported source for the shopper's market is eligible, including official brand stores, direct retailers, electronics retailers, authorized reseller sites, and other marketplaces where a legitimate data path exists.
- Search inclusion depends on legitimate data access (official API, authorized product feed, approved affiliate/product feed, direct retailer/partner integration, permitted public data source, or another contractually/technically legitimate source). Affiliate status is not the inclusion test. Scraping is not a workaround for unavailable rights.
...
- PH source certification must record effective-cost data capabilities using the existing Sprint 31 capability-policy states (`allowed` / `restricted` / `prohibited` / `unknown`). Provider approval and affiliate approval do not imply permission for listing price, discount, voucher, shipping, or checkout-cost capabilities.
...
| 32 | Philippines Merchant Certification | ≥1 real PH path. Initial validation market. Record effective-cost capabilities. TikTok not launch-critical. | P0-1 (PH) | EXT-01,06,07 | Real legally usable current-data response + capability-policy evidence including effective-cost data capabilities; staging+limited proof. Fixtures cannot certify. Shopee/Lazada are not certified by this lock. |
...
| 37 | MarketContext, Currency & Localization | Coherent market/FX/locale + shipping honesty + destination re-evaluation | P1-1B; P1-2 | EXT-23 | 37.1–37.2 merged; 37.3 currency authority + conversion-unavailable foundation implemented. Production FX conversion remains **unavailable**. Production certified shopping markets remain **zero**. EXT-23 stays `not_started` (no live provider, credentials, or production quotes). Sprint 37 is **not complete**. Exit still requires a trusted FX source, five-market/unsupported-market completion, shipping honesty closure, and server-side re-evaluation when destination materially changes cost; FR-CA decision recorded |
...
- Official brand stores, direct retailers, electronics retailers, authorized reseller sites, and other marketplaces remain fully eligible for search, PiqScore, Best Piq, and Recommendation when a legitimate data path exists — including when PiqSavi earns ₱0.
...
| EC-31 | Frozen launch candidate uses verified applicable purchase-cost components for effective cost / ranking, remains affiliate-neutral, and does not treat unknown costs as zero | 45 verifies *(implementation: 29 economics + 32 capability recording + 37 shipping/destination honesty; neutrality: 5/6/20/44)* | Effective-purchase-cost evidence package on the frozen candidate (see §9.5). Use only capabilities the certified sources actually expose. | No-go if sticker-only ranking is used when verified components exist; if unverified/conditional vouchers affect PiqScore; if unknown shipping becomes ₱0/FREE; if affiliate commission affects the result; or if a non-affiliate merchant is suppressed solely for lacking affiliate |
...
Do **not** require a merchant to expose capabilities it does not provide. In that case PiqSavi must expose the limitation honestly. Evidence must use the actual certified merchant capabilities available at launch.
...
| Searches stores worldwide | ≥1 real path in each named market | 32–36 / 44 | “Searches supported merchants in [list]” / “Best Piq among the offers PiqSavi evaluated” | “Searches stores worldwide”; implying TikTok or any unqueried marketplace was searched; implying Shopee + Lazada are the only merchants considered |
...
| 9a | Affiliate-neutral merchant coverage | 5/6/20 + 32–36 + 44/45 | Engine verified; PH sources uncertified | Non-affiliate merchants remain fully eligible for PiqScore / Best Piq / Recommendation; Shopee + Lazada are initial affiliate-monetization targets, not exclusive search | Yes — NON-WAIVABLE | Organic links; earn ₱0 if a non-affiliate wins |
| 9b | Effective-cost capability recording | 32–36 (PH: 32); contract: 31 | Planned at certification | Each PH source records listing/discount/voucher/shipping/checkout/freshness capabilities via Sprint 31 policy states | Yes to certify that source | Disclose unavailable capabilities honestly |
