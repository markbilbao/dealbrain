...
| 27 | Transactional Identity & Email | Real email; reset/verify/email-change; token lifecycle; enumeration-safe errors; session rotation | P0-5; HIGH demo-auth | EXT-08, EXT-09 | **In progress — not complete.** 27.1–27.4 implemented. 2026-09-08 staging inbox E2E (verify/reset/email-change) passed; auth-aware header/logout passed. EXT-09 public DNS resolvable; Resend domain **Verified** not independently established. Production secret attach remains Sprint 41. Evidence: [`evidence/SPRINT_27_STAGING_EMAIL_EVIDENCE_2026-09-08.md`](evidence/SPRINT_27_STAGING_EMAIL_EVIDENCE_2026-09-08.md) |
...
3. **Sprint 27** — real transactional email, sender auth, verify/reset/email-change, token lifecycle, enumeration-safe errors, session rotation, staging E2E, production cutover readiness. Staging inbox E2E passed 2026-09-08; EXT-09 Resend **Verified** evidence still required.
...
See §8 and the register. The items that can miss September 30 unless resolved or scope-reduced are currently **RED/AMBER**: PH product-data access (RED until ≥1 certified PH market), email DNS/auth EXT-09 Resend Verified (AMBER; staging inbox delivery evidenced 2026-09-08), counsel written approval + policy publication (AMBER), production DNS/TLS/AWS/secrets (AMBER), paging (AMBER), Search Console (AMBER), FX (AMBER, reducible), AI quota (AMBER, reducible). EXT-02…05 are **GREEN** for September (`n_a_beta`). EXT-07 affiliate tracking is optional, `n_a_beta`, and is **not** a September launch blocker (2026-09-07).
