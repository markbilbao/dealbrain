...
| EXT-11 | DNS for public hostname | Ops | 41 | production infrastructure | After EXT-10 | 1–3 days | `provisioned` | **GREEN** for public hostname resolution | Apex + www resolve to production ALB `dealbrain-production-alb-1597181926.us-east-1.elb.amazonaws.com` (read-only 2026-09-18). www canonical 301 remains a Sprint 41 gap. Historical `not_started` notes below are not current. | Delay public access | Public web access |
| EXT-12 | TLS certificate (ACM or equivalent) | Ops | 41 | production infrastructure | After DNS | 1–7 days | `provisioned` | **GREEN** for HTTPS | Live Amazon RSA cert SAN `piqsavi.com` + `www.piqsavi.com`; HTTP→HTTPS 301; HTTPS `/ready` 200 (2026-09-18). Historical `not_started` notes below are not current. | Delay public access | Public web access |
| EXT-13 | AWS production account/resources | Ops | 41 | production infrastructure | Ongoing | Continuous | `provisioned` | **GREEN** for live production stack; Sprint 41 still only partially complete | Account `941035169846` / `us-east-1`; Deploy Production #6 `production_ok`; live `/ready` READY. Historical “TF complete; not applied” notes below remain historical. | Cannot launch production | Entire production launch |
| EXT-14 | Production secrets populated | Ops | 41 | production infrastructure | With EXT-13 | 1–5 days | `provisioned` | **GREEN** for operational attach (values never printed) | Deploy #6 succeeded; live `/health` `identity_email_ready=true`. No secret values read or written in the 2026-09-18 reconciliation. Redacted env dump still not filed. | Cannot deploy prod | Entire production launch |
...
### 2026-09-18 Sprint 41 production reconciliation addendum

This addendum does **not** rewrite the 2026-09-11 cutover re-audit or Phase 1 “not applied” notes. Those remain historical. Current-status rows for EXT-11…14 are updated in the register table above.

| ID | Status after 2026-09-18 read-only audit | Live evidence |
|----|------------------------------------------|---------------|
| EXT-10 | remains `approved` (ownership). Public usability is EXT-11/12 | Unchanged ownership screenshot |
| EXT-11 | **`provisioned`** | `piqsavi.com` and `www.piqsavi.com` resolve to `dealbrain-production-alb-1597181926.us-east-1.elb.amazonaws.com`. www→apex 301 is **not** in place |
| EXT-12 | **`provisioned`** | Amazon RSA cert SAN apex+www; HTTP→HTTPS 301; HTTPS `/ready` 200 |
| EXT-13 | **`provisioned`** | Account `941035169846` / `us-east-1`; Deploy Production #6 `production_ok`; instance `i-0e49ddadd4ecc772d` at that deploy |
| EXT-14 | **`provisioned`** (existence; values never printed) | Deploy #6 required assembled secrets; live `/health` `identity_email_adapter=resend` / `identity_email_ready=true` |
| EXT-20 / EXT-21 | remain `applied` | Production `https://piqsavi.com/privacy` and `/terms` HTTP 200 on 2026-09-18. Status stays `applied` (content-layer; not unconditional counsel approval). Staging URL was last verified Deploy Staging #35; staging is currently 503 |
| EXT-08 / EXT-09 | unchanged taxonomy (`applied` / `approved`) | Production Resend attach is operational via `/health`. Register screenshot taxonomy is not upgraded from this health probe |

Evidence: [`evidence/SPRINT_41_CURRENT_MAIN_RECONCILIATION_2026-09-18.md`](evidence/SPRINT_41_CURRENT_MAIN_RECONCILIATION_2026-09-18.md); [`evidence/PRODUCTION_DEPLOY_6_EVIDENCE_2026-09-15.json`](evidence/PRODUCTION_DEPLOY_6_EVIDENCE_2026-09-15.json).

Sprint 41 remains **not** COMPLETE/CLOSED. No production mutation was performed in that reconciliation.
