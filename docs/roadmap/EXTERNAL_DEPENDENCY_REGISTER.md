...
| EXT-06 | Merchant credentials (named markets) | Ops + marketplace | 32 (PH); 33–36 if later expanded | global launch *(per named market that requires credentials)* | After approval, when the selected path requires credentials | 1–2 weeks | `not_started` | **RED** for credential-required paths; **NOT APPLICABLE** to the S-1 Anonymous Shopify Global Catalog path; not a multi-market September blocker | Secrets Manager entries when a selected path requires credentials; no plaintext in git. Shopify Global Catalog Anonymous catalog mode does not require merchant/API credentials. Do not mark this row `provisioned` from that mode. September named-market target is PH only. | Credential-required paths cannot certify without credentials. Lack of credentials must not block the documented S-1 Anonymous Shopify Global Catalog path when its actual certification requirements pass. | Credential-required named-market paths |
...
- EXT-01 / EXT-06 (PH) — EXT-01 is now `applied` on 2026-09-08 Lazada PH and Shopee PH product-data request evidence. That is **not** approval, credentials, a feed, or certification. EXT-06 remains `not_started` and is **not applicable** to the documented S-1 Anonymous Shopify Global Catalog path, which does not require merchant/API credentials. Lack of credentials must not block that path. Do not mark EXT-06 `provisioned`. Counsel clearance to apply (2026-08-25) is historical. Shopee Affiliate dashboard access and Payment & Tax pending do **not** make EXT-01 `approved`. Affiliate Open API access is **not granted**. Lazada/Optimise affiliate approval, if any, is monetization permission only and does **not** certify a live research source. Naming PH remains blocked until a legitimate path's actual certification requirements pass. September public shopping launch remains blocked until **at least one useful PH product-data path** is certified. US/SG/UK/CA are **not** September substitutes unless the owner later expands supported markets.
...
| Merchant-approved product-data path? | **No** for the 2026-09-08 Shopee and Lazada requests — those requests are still awaiting an external provider decision. This field does not invent a separate application or preapproval for the documented Shopify Global Catalog Anonymous catalog mode. |
| Product-data / API rights | **unknown / not established** for Shopee and Lazada. Shopify Anonymous catalog-mode evidence is recorded separately and is not an EXT-01 approval. |
| Credentials | **No** Shopee or Lazada credentials. Shopify Global Catalog Anonymous catalog mode does not require merchant/API credentials. |
...
| Next owner / Sprint 32 action | Await a Shopee or Lazada provider response where those paths are selected. Sprint 32 still requires approval, credentials, and feed/API availability **where the selected path requires them**. The documented Anonymous Shopify Global Catalog path does not require those stages. Remaining Shopify certification gates still include production profile deployment, an executable production provider, trusted production certification, canonical normalization evidence, staging certification, monitoring/coverage disclosure, and kill-switch closure evidence. |
...
A named market needs a legitimate authorized access path whose actual access requirements are satisfied. That may be established by either:

A. Provider approval, provisioning, and credentials where the selected path requires those things. Affiliate-only approval is not enough. `applied` is not `approved` or `provisioned`.

OR

B. Official provider documentation establishing a public, keyless, or anonymous access mode with no separate credential or preapproval requirement, plus the remaining certification evidence.

The remaining certification evidence is still required in both cases: a real, legally usable, current-data response; Sprint 31 capability-policy evidence that is declared, evidence-backed, and fail-closed enforced; truthful coverage disclosure; and the certification sprint exit gate. Satisfied access stages do not by themselves name the market.

This exception applies only to the actual documented path. It does not weaken Shopee, Lazada, or any other provider whose selected path requires application, preapproval, or credentials. EXT-01 remains `applied` for the 2026-09-08 Shopee and Lazada requests and is not `approved` or `provisioned`. EXT-06 remains `not_started` and is not applicable to the documented S-1 Anonymous Shopify Global Catalog path.
...
| EXT-06 | Technical credentials available in the target environment when the selected path requires credentials | Contractual/policy authorization for every capability. Not applicable to the documented S-1 Anonymous Shopify Global Catalog path, which does not require merchant/API credentials. `not_started` is not `provisioned`. |
...
### 2026-09-23 Shopify Anonymous catalog access-stage addendum (statuses unchanged)

This addendum does **not** rewrite the 2026-09-08 EXT-01 close. It records path applicability for one documented catalog mode.

- EXT-01 remains `applied` for the 2026-09-08 Shopee Philippines and Lazada Philippines product-data requests. Shopee is not approved. Lazada is not approved. Neither is provisioned. Shopify evidence does not move EXT-01 to `approved`.
- EXT-06 remains `not_started`. It is the generic credentials dependency for paths that require credentials. It is **NOT APPLICABLE** to the S-1 Anonymous Shopify Global Catalog path. Shopify Global Catalog Anonymous catalog mode does not require merchant/API credentials. Lack of credentials must not block that path if its actual certification requirements pass. Do not mark EXT-06 `provisioned`. Do not fabricate credentials.
- The documented Anonymous mode still requires an agent profile. Staging profile deployment and the later staging Shopify negotiation are technical evidence. They are not production certification, a partnership, an endorsement, special approval, preferred-developer status, or production-app approval.
- Signed and Token identification tiers are optional stronger modes. They are not required to prove Anonymous catalog-tool access.
- Promoted placement remains a different path: not enrolled, disabled, policy unknown.
