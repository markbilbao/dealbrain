...
The capability-policy preparation map is documentary only. It uses existing Sprint 31 states (`allowed` / `restricted` / `prohibited` / `unknown`). It does not create a second policy system and does not populate production provider, certification, or routing registries. Technical exposure is not permission. Unknown permissions are not marked allowed. No row is `allowed`. Query-time comparison uses `restricted`. Persistent indexing, search-result caching, AI training/model improvement, promoted placement, affiliate ranking, `lookup_catalog` in this probe, and raw response persistence are `prohibited`. Discount, voucher, shipping-amount, free-shipping, and checkout-cost permissions remain `unknown`. Canonical shopper/offer applicability is not established.
