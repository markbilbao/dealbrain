...
- Verify (do not implement) EC-31 effective-purchase-cost honesty and affiliate neutrality: non-affiliate merchants remain eligible for Best Piq; affiliate status never excludes or privileges an otherwise relevant legitimate source; unverified/conditional vouchers do not affect PiqScore; affiliate commission does not affect the result. Mixed affiliate/non-affiliate **runtime** comparison is required only if affiliate-enabled merchants are active in the frozen candidate; if zero are active, architecture/tests must still prove affiliate economics are absent from organic scoring/recommendation paths
- Verify (do not implement) that launch UI does not show active-affiliate disclosure (“Affiliate link”, “PiqSavi may earn a commission”) next to ordinary outbound merchant links when no affiliate relationship is actually active for that action. Keep the Affiliate & Advertising Disclosure as a future-capable legal document; do not publish it in this sprint; do not claim counsel approval beyond recorded evidence
- Verify (do not implement) that September launch claims do not treat Shopee/Lazada affiliate approval, Optimise tracking, or EXT-07 as launch blockers, and do not represent affiliate approval as product-data permission
