...
**Does not** implement a CMP/banner (EXT-22 remains `not_started`; Sprint 39 owns activation).
...
| `localStorage` / `sessionStorage` **NOT FOUND** | `sessionStorage` key `piqsavi_ask_conversation` stores a `conversation_id`. Consumer account JS also stores `piqsavi_access_token` in `sessionStorage` or `localStorage` (remember-me) and `piqsavi_remember_me` in `localStorage`. These are first-party functional/auth device stores, not third-party analytics identifiers |
...
| No CMP/banner | **Unchanged** — none added. Essential-only fail-closed hook exists (`app/privacy/tracking.py`); non-essential categories remain OFF; Sprint 39 owns CMP/analytics activation |
